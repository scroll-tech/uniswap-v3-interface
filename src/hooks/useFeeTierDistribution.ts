...
function usePoolTVL(token0: Token | undefined, token1: Token | undefined) {
  ...
  return useMemo(() => {
    ...
    const all = asToken0.concat(asToken1).filter((value): value is NonNullable<typeof value> => value !== null)
    ...
    const tvlByFeeTier = all.reduce<{ [feeAmount: number]: [number | undefined, number | undefined] }>(
      (acc, value) => {
        if (value.feeTier === null) return acc
        const feeTier = Number(value.feeTier)
        if (!acc[feeTier]) {
          acc[feeTier] = [undefined, undefined]
        }
        acc[feeTier][0] = (acc[feeTier][0] ?? 0) + Number(value.totalValueLockedToken0 ?? 0)
        acc[feeTier][1] = (acc[feeTier][1] ?? 0) + Number(value.totalValueLockedToken1 ?? 0)
        return acc
      },
      {
